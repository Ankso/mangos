    ...
    switch(m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            else if (m_spellInfo->Id == 49576)
            {
                ...
                if (unitTarget->GetObjectGuid().IsVehicle()) // Don't allow do this to players on vehicles or to vehicles.
                    return;
...
void Spell::EffectLeapForward(SpellEffectIndex eff_idx)
{
    ...
    if( m_spellInfo->rangeIndex == 1)                       //self range
    {
        ...
        for (float i=0.5f; i<dis; i+=0.5f)
        {
            ...
            dz = unitTarget->GetMap()->GetTerrain()->GetHeight(dx, dy, cz, useVmap);
...
void Spell::EffectCharge(SpellEffectIndex /*eff_idx*/)
{
    ...
    // Try to normalize Z coord cuz GetContactPoint do nothing with Z axis
    if(!m_caster->GetMap()->GetTerrain()->IsNextZcoordOK(x, y, z, 30.0f))
    {
...
void Spell::EffectDamageBuilding(SpellEffectIndex eff_idx)
{
    ...
    gameObjTarget->DamageTaken(m_caster, damage);
}

    ...
    if (DalaranRestrictedFlightArea >= 1)
    {
        if (GetMapId() == 571 && IsInWorld())
        {
            if (GetPositionZ() > 640.0 && GetPositionZ() < 700.0 && GetZoneId() == 4395 && GetAreaId() != 4564 && CanFly() && !isGameMaster())
            {
                if (!HasAura(58600) && !HasAura(61243)) // Check for Restricted Flight Area and Parachute Visual
                {
                       ...
                       GetAreaId() == 4619))                                                                                               // The Violet Citadel
                    {
                        ...
                        MonsterWhisper("Advertencia: Has entrado en una zona de vuelo restringido y vas a ser desmontado.", this, true);
                    }
                }
            }
            ...
            if (HasAura(61243)) // Parachute Visual
            {
                float x, y, z;
                GetPosition(x, y, z);
                float ground_Z = GetMap()->GetTerrain()->GetHeight(x, y, z, true);
...
bool Player::LoadFromDB(ObjectGuid guid, SqlQueryHolder *holder )
{
    ...
    //"health, power1, power2, power3, power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, xp_rate  FROM characters WHERE guid = '%u'", GUID_LOPART(m_guid));
...
void Player::SaveToDB()
{
    ...
    ss << "REPLACE INTO characters (guid,account,name,race,class,gender,level,xp,money,playerBytes,playerBytes2,playerFlags,"
        ...
        "power4, power5, power6, power7, specCount, activeSpec, exploredZones, equipmentCache, ammoId, knownTitles, actionBars, grantableLevels, xp_rate) VALUES ("
        << GetGUIDLow() << ", "
    ...
    //CharacterDatabase.PExecute("UPDATE `characters` SET `xp_rate` = '%u' WHERE `guid` = '%u'", xp_rate, GetGUIDLow());

...
pAuraHandler AuraHandler[TOTAL_AURAS]=
{
    ...
    &Aura::HandleAuraAddMechanicAbilities,                  //293 replaces target's action bars with a predefined spellset
    ...
    else
    {
        ...
        switch(GetId())
        {
            ...
            case 58600:                                     // Restricted Flight Area
            {
                ...
                {
                    target->CastSpell(target, 58601, true);   // Remove Flight Auras
                    target->CastSpell(target, 45472, true);   // Parachute Buff
                    target->CastSpell(target, 61243, true);   // Parachute Visual
                    return;
                }
            }
...
void Aura::HandleAuraModDisarmOffhand(bool apply, bool Real)
{
    ...
    if(apply)
        target->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISARM_OFFHAND);
    else
        target->RemoveFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISARM_OFFHAND);
    ...
    if(Item *_item = ((Player*)target)->GetItemByPos( INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_OFFHAND ))
        ((Player*)target)->_ApplyItemMods(_item, EQUIPMENT_SLOT_OFFHAND, !apply);
...
void Aura::HandleAuraModDisarmRanged(bool apply, bool Real)
{
    ...
    // not sure for it's correctness
    if(apply)
        target->SetFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISARM_RANGED);
    else
        target->RemoveFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_DISARM_RANGED);
    ...
    if(Item *_item = ((Player*)target)->GetItemByPos( INVENTORY_SLOT_BAG_0, EQUIPMENT_SLOT_RANGED ))
        ((Player*)target)->_ApplyItemMods(_item, EQUIPMENT_SLOT_RANGED, !apply);
